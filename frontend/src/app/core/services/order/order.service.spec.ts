...
describe('OrderService', () => {
  ...
  it('should start with initial dummy orders', () => {
  ...
  it('should place an order and return it with status PENDING', async () => {
    const order = await firstValueFrom(service.placeOrder(mockCartItems, 'dine-in', 5));
  ...
  it('should NOT cancel an order that is not PENDING', async () => {
    ...
    await expect(firstValueFrom(service.cancelOrder(placed._id))).rejects.toThrow();
  });
  ...
  it('should update order status from PENDING to IN_PREPARATION', async () => {
    ...
    const updated = await firstValueFrom(service.updateStatus(placed._id, 'IN_PREPARATION'));

    expect(updated.status).toBe('IN_PREPARATION');
  });

  it('should reflect status changes in the orders signal', async () => {
    ...
    const order = service.orders().find((o) => o._id === placed._id);
